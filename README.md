...
`counter/` 是自建的访问计数器（Cloudflare Worker + D1，免费额度），部署在 `https://gh-visits.antony138.workers.dev`，本站和 mainichi-sanku 共用（各自独立计数）。前端 localStorage 判定新访客，不采集 IP、不种 cookie。改动后在 `counter/` 目录 `npx wrangler deploy` 即可重新部署。

## 成本

零现金：GitHub Pages 免费 + routine 走 Claude 订阅额度（每天一次几分钟的小会话）+ 计数器在 Cloudflare 免费额度内。
